...
// Khởi tạo mảng để chứa các bản ghi
$subjects = [];
...
<div class="table-content">
    <table id="collapsetable" class="collapsetable" border="1">
        ...
        $day = $_COOKIE['day'];
        $id = $_COOKIE['id'];
        $sql = "SELECT * FROM `period` WHERE userId = '$id' AND `day` = '$day' ORDER BY `std` ";
        ...
        <tr class="center">
            ...
            <td><?php echo $row["status"] == 1 ? 'REGISTERED' : 'NOT REGISTERED'; ?></td>
            <td><a style="margin: auto; padding: 7px;" class="btn"
                   onclick="return confirm('Are you sure to delete the period?')"
                   href="del.php?id=<?php echo $row['no'] ?>">DELETE</a></td>
